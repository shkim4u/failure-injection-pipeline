...
def eks_single_az_failover(autoscaling_client, vpc_id, az_name, tag_name, tag_value):
    ...
    for cluster in eks_cluster_list['clusters']:
        ...
        if cluster_response['cluster']['resourcesVpcConfig']['vpcId'] == vpc_id:
            logger.info('EKS Single-AZ Failover cluster found in target vpc: %s', cluster)
...
def run(region, az_name, vpc_id, duration, tag_name, tag_value, limit_asg, failover_rds, failover_elasticache, log_level='INFO'):
    global original_asg
